...
export const HeroSection = () => {
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br from-primary/10 via-background to-background px-6 py-20 shadow-glow-strong">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(16,185,129,0.12),transparent_65%)]" />
      <div className="pointer-events-none absolute -left-24 top-12 hidden h-64 w-64 animate-float-slow rounded-full bg-primary/20 blur-3xl lg:block" />
      <div className="pointer-events-none absolute bottom-10 right-10 h-56 w-56 animate-pulse-soft rounded-full bg-primary/10 blur-3xl" />
      <div className="pointer-events-none absolute bottom-0 left-1/2 hidden h-[220px] w-[220px] -translate-x-1/2 animate-float-slow rounded-full bg-primary/15 blur-2xl md:block" style={{ animationDelay: "1.2s" }} />

      <div className="relative z-10 mx-auto flex max-w-6xl flex-col gap-14 lg:flex-row lg:items-center lg:gap-24">
        <div className="flex-1 space-y-8">
          <span className="inline-flex animate-fade-in items-center gap-2 rounded-full border border-primary/40 bg-background/90 px-5 py-2 text-sm text-primary shadow-sm">
            <Sparkles className="h-4 w-4" />
            {t("hero.badge")}
          </span>

          <div className="space-y-4">
            <h1 className="animate-slide-up text-4xl font-display font-bold tracking-tight text-foreground sm:text-5xl lg:text-6xl">
              {t("hero.title")}
            </h1>
        ...
        <div className="flex-1 space-y-6 rounded-2xl border border-primary/20 bg-card/80 p-6 backdrop-blur">
          <div className="grid gap-6 sm:grid-cols-3">
            {stats.map((stat, index) => (
              ...
              >
                <p className="text-3xl font-display font-semibold text-primary">{stat.value}</p>
